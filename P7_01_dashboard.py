...
def user_input_features():
    EXT_SOURCE_3 = st.slider('EXT_SOURCE_3', 0.03, 0.83, 0.40)
    EXT_SOURCE_2 = st.slider('EXT_SOURCE_2', 0.03, 0.83, 0.40)
    DAYS_EMPLOYED = st.slider('DAYS_EMPLOYED', -12203.00, -100.00, -6000.00)
    AMT_REQ_CREDIT_BUREAU_YEAR = st.slider('AMT_REQ_CREDIT_BUREAU_YEAR', 0.00, 8.00, 4.00)
    OWN_CAR_AGE = st.slider('OWN_CAR_AGE', 1.00, 30.00, 15.00)
    OBS_60_CNT_SOCIAL_CIRCLE = st.slider('OBS_60_CNT_SOCIAL_CIRCLE', 0.00, 10.00, 5.00)
    OBS_30_CNT_SOCIAL_CIRCLE = st.slider('OBS_30_CNT_SOCIAL_CIRCLE', 0.00, 10.00, 5.00)
    AMT_ANNUITY = st.slider('AMT_ANNUITY', 6750.00, 67500.00, 30000.00)
    DAYS_REGISTRATION = st.slider('DAYS_REGISTRATION', -14166.00, -51.00, -7000.00)
    DAYS_LAST_PHONE_CHANGE = st.slider('DAYS_LAST_PHONE_CHANGE', -2800.00, 0.00, -1400.00)
    EXT_SOURCE_1 = st.slider('EXT_SOURCE_1',0.06, 0.88, 0.44)
    REGION_POPULATION_RELATIVE = st.slider('REGION_POPULATION_RELATIVE', 0.00, 0.07, 0.04)
    AMT_INCOME_TOTAL = st.slider('AMT_INCOME_TOTAL', 45000.00, 990000.00, 500000.00)
    HOUR_APPR_PROCESS_START = st.slider('HOUR_APPR_PROCESS_START', 4.00, 20.00, 12.00)
    AMT_GOODS_PRICE = st.slider('AMT_GOODS_PRICE', 67500.00, 1800000.00, 900000.00)
    DAYS_BIRTH = st.slider('DAYS_BIRTH', 7712.00, 25010.00, 15000.00)
    DAYS_ID_PUBLISH = st.slider('DAYS_ID_PUBLISH', -6021.00, -20.00, -3000.00)
    AMT_CREDIT = st.slider('AMT_CREDIT', 67500.00, 2013840.00, 1000000.00)
    SK_ID_CURR = st.slider('SK_ID_CURR', 103679.00, 448119.00, 250000.00)
