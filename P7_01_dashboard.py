...
def user_input_features():
    EXT_SOURCE_3 = st.sidebar.slider('EXT_SOURCE_3', 0.03, 0.83, float(0.40))
    EXT_SOURCE_2 = st.sidebar.slider('EXT_SOURCE_2', 0.03, 0.83, float(0.40))
    DAYS_EMPLOYED = st.sidebar.slider('DAYS_EMPLOYED', -12203.00, -100, float(-6000))
    AMT_REQ_CREDIT_BUREAU_YEAR = st.sidebar.slider('AMT_REQ_CREDIT_BUREAU_YEAR', 0.00, 8.00, float(4))
    OWN_CAR_AGE = st.sidebar.slider('OWN_CAR_AGE', 1.00, 30.00, float(15))
    OBS_60_CNT_SOCIAL_CIRCLE = st.sidebar.slider('OBS_60_CNT_SOCIAL_CIRCLE', 0.00, 10.00, float(5))
    OBS_30_CNT_SOCIAL_CIRCLE = st.sidebar.slider('OBS_30_CNT_SOCIAL_CIRCLE', 0.00, 10.00, float(5))
    AMT_ANNUITY = st.sidebar.slider('AMT_ANNUITY', 6750.00, 67500.00, float(30000))
    DAYS_REGISTRATION = st.sidebar.slider('DAYS_REGISTRATION', -14166.00, -51.00, float(-7000))
    DAYS_LAST_PHONE_CHANGE = st.sidebar.slider('DAYS_LAST_PHONE_CHANGE', -2800.00, 0.00, float(-1400))
    EXT_SOURCE_1 = st.sidebar.slider('EXT_SOURCE_1',0.06, 0.88, float(0.44))
    REGION_POPULATION_RELATIVE = st.sidebar.slider('REGION_POPULATION_RELATIVE', 0.00, 0.07, float(0.04))
    AMT_INCOME_TOTAL = st.sidebar.slider('AMT_INCOME_TOTAL', 45000.00, 990000.00, float(500000))
    HOUR_APPR_PROCESS_START = st.sidebar.slider('HOUR_APPR_PROCESS_START', 4.00, 20.00, float(12))
    AMT_GOODS_PRICE = st.sidebar.slider('AMT_GOODS_PRICE', 67500.00, 1800000.00, float(900000))
    DAYS_BIRTH = st.sidebar.slider('DAYS_BIRTH', 7712.00, 25010.00, float(15000))
    DAYS_ID_PUBLISH = st.sidebar.slider('DAYS_ID_PUBLISH', -6021.00, -20.00, float(-3000))
    AMT_CREDIT = st.sidebar.slider('AMT_CREDIT', 67500.00, 2013840.00, float(1000000))
    SK_ID_CURR = st.sidebar.slider('SK_ID_CURR', 103679.00, 448119.00, float(250000))
